    ...
    * stopwords
    ...
    def cleaned(self) -> str:
        ...
        * If a list of extract objects were given, remove those too
        ...
        """
        cleaned = ''.join([c for c in self.txt
                           if c not in string.punctuation]).lower()

        if self.extra:
            for char in self.extra:
                update_text = cleaned.replace(char, ' ')
                cleaned = update_text
    ...
    def metrics(self) -> List[Tuple[str, int]]:
        ...
        * Use count to return the requested amount of the top words, defaults to 5
        ...
        """
        count = Counter(
            [t for t in self.cleaned.split() if t not in self.stopwords])
    ...
    def graph(self) -> None:
        ...
        * The tag is repeated the number of counts of the word
        ...
        for name, count in results:
            space = ' ' * (10 - len(name))
            text = f"{space}{name} {self.tag*count}"
            print(text)


if __name__ == "__main__":
    corp = Corpora(GETTYSBURG)
    corp.extra = EXTRA_CHAR
    # corp.cleaned
    # corp.metrics
    # corp.tag = '*'
    corp.graph
