...
def years_gold_value_decreased(gold_prices: str = gold_prices) -> (int, int):
    ...
    gold_prices: List[str, str] = gold_prices.strip().split()

    c_date, c_price = gold_prices[0].split(',')

    for g in gold_prices[1:]:
        g_date, g_price = g.split(',')

        diff = float(g_price) - float(c_price)

        res.append((int(g_date[:4]), diff))

        c_date = g_date
        c_price = g_price

    return (min(res, key=lambda x: x[1])[0], max(res, key=lambda x: x[1])[0])
...
if __name__ == "__main__":
    ...
    print(years_gold_value_decreased(data_b))
    print(years_gold_value_decreased(data_c))