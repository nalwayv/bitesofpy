...
def test_new_signs_are_signs(signs):
    for sign in signs:
        assert str(type(sign))[-6:-2] == "Sign"


def test_signs_are_mutually_compatible_are_in_compatible(signs):
...
def test_most_famous_sign(signs):
...
def test_get_sign_by_date_in_range(signs):
    sign = 'Scorpio'
